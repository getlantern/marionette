...
func TestEncodeDecode(t *testing.T) {
	t.Run("Simple", func(t *testing.T) {
		...
		if v, err := EncodeDecode(`^(a|b)+$`, plaintext); err != nil {
			t.Fatal(err)
		} else if v != plaintext {
			t.Fatalf("expected %q, got %q", plaintext, v)
		}
	})
}

// EncodeDecode encodes and the decodes plaintext and returns the result.
func EncodeDecode(regex, plaintext string) (string, error) {
	ciphertext, err := fte.Encode(regex, plaintext)
	if err != nil {
		return "", fmt.Errorf("encode error: %s", string(err.(*exec.ExitError).Stderr))
	}

	output, err := fte.Decode(regex, ciphertext)
	if err != nil {
		return "", fmt.Errorf("decode error: %s", string(err.(*exec.ExitError).Stderr))
	}
	return output, nil
}
